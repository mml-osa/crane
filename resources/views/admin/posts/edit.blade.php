...
@section('content')
    ...
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">Edit Post | <small>edit website post</small></h1>
    <!-- END PAGE TITLE-->
    ...
    <div class="row">
                    ...
                    <div class="portlet-body form">
                        ...
                        <div class="col-md-12 text-right">
                            <div class="form-actions noborder">
                                ...
                                <button type="button" data-toggle="modal" data-target="#destroy{{$posts->id}}" href="#destroy{{$posts->id}}" class="btn btn-sm btn-danger" @if($session->userRole['alias'] != 'administrator') disabled @endif>Remove</button>
                            </div>
                        </div>
                        ...
                        <div class="form-body">
                            ...
                            <div class="col-md-12">
                                <div class="form-group form-md-line-input form-md-floating-label">
                                    <input name="title" type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{ $posts->title ?? old('title') }}" id="title" autofocus autocomplete="title" required />
                            ...
                            <div class="col-md-12">
                                <div class="form-group form-md-line-input form-md-floating-label">
                                    <input name="caption" type="text" class="form-control{{ $errors->has('caption') ? ' is-invalid' : '' }}" value="{{ $posts->caption ?? old('caption') }}" id="caption" autocomplete="caption" />
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea rows="8" name="content" id="summernote_1" placeholder="Your content text here" class="form-control{{ $errors->has('content') ? ' is-invalid' : '' }}">{{ $posts->content ?? old('content') }}</textarea>
                            </div>
                        </div>
                    ...
                    <div class="portlet-body">
                        <div class="tab-content">
                            <div class="tab-pane active" id="portlet_tab2_1">
                                <p><b>Published: </b><input type="radio" name="published" id="yes" value="1" @if($posts->published == 1) checked @endif> <label for="yes">Yes</label> &emsp;<input type="radio" name="published" id="no" value="0" @if($posts->published == 0) checked @endif> <label for="no">No</label></p>
                                <p><b>Visibility: </b><br>
                                    @foreach ($visibility as $visible)
                                        <input type="radio" name="visibility_id" id="{{$visible->alias}}" value="{{$visible->id}}" @if($posts->visibility_id == $visible->id) checked @endif> <label for="{{$visible->alias}}">{{$visible->title}}</label><br>
                                    @endforeach
                                </p>
                                <p><b>Created At: </b><a href="#create_date" data-target="#create_date" data-toggle="modal" title="Click to change post date">@if($posts->create_date != null) {{ ($posts->create_date) }} @else {{ now() }} @endif</a></p>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="portlet_tab2_1">
                                @foreach ($postCategories as $postCategory)
                                    <input type="checkbox" name="cat_id" id="cat_id{{$postCategory->id}}" value="{{$postCategory->id}}" @if($posts->cat_id == $postCategory->id) checked @endif>
        ...
        <div class="modal fade" id="destroy{{$posts->id}}" tabindex="-1" role="form" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                    ...
                    <div class="modal-body text-center" style="color: red"> Are you sure you want to remove this post? </div>
                    <div class="modal-footer">
                        <form id="delete-user" action="{{ route('posts.destroy',$posts->id) }}" method="post">
                            @csrf
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="portlet_tab2_1">
                            ...
                            @foreach($postMedia as $media)
                                @if($media->post_id == $posts->id  && $media->mediaType['alias'] == "image")
                                    @foreach($items as $item)
                                        @if($media->item_id == $item->id)
                                            ...
                                            @if($item->mediaAlbum['sub'] == 1)
                                                <?php $alias = $item->mediaAlbum->mediaAlbum['alias']?>
                                                <a data-toggle="modal" href="#img_remove{{$media->id}}" title="click to remove image"><img src="{{asset("storage/app/public/web/album/$alias/$album/$item->file")}}" alt="" width="100%"></a>
                                            @else <a data-toggle="modal" href="#img_remove{{$media->id}}" title="click to remove image"><img src="{{asset("storage/app/public/web/album/$album/$item->file")}}" alt="" width="100%"></a> @endif
                                            ...
                                            <div class="modal fade" id="img_remove{{$media->id}}" tabindex="-1" role="form" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <form id="delete-item" action="{{ route('posts.featured.remove',$media->id) }}" method="POST">
                                                                @csrf
                            ...
                            <div class="clearfix"></div>
                            <br><a class="btn btn-danger btn-xs" data-target="#featuredAll{{$posts->id}}" data-toggle="modal" title="select image">Remove All</a>
                            <div class="modal fade" id="featuredAll{{$posts->id}}" tabindex="-1" role="form" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                                        ...
                                        <div class="modal-footer">
                                            <form id="delete-item" action="{{route('posts.featured.all',$posts->id)}}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-sm red">Remove</button>
                                                <button type="button" class="btn btn-sm dark btn-outline" data-dismiss="modal">Close</button>
                                            </form>
